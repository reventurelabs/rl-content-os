...
Run the `rl-voice-discovery` skill now. The skill file is the source of truth; where this summary and the skill differ, the skill wins.
...
4. **Validate.** Write a short test passage on a neutral topic, get direct feedback on where it doesn't sound right, fold in corrections — and if the gap was significant, run a second test passage before finalizing.
5. **Hand off.** `rl-content-pipeline` and `rl-writing-craft` already look for this file wherever they reference "your author-voice skill" — nothing further to wire up.
